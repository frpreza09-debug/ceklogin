...
.site-header{
  ...
  padding:18px 60px;background:#0f172a;
  border-bottom:1px solid rgba(0,242,255,0.12);
  position:sticky;top:0;z-index:100;
}
.logo{font-size:20px;font-weight:800;color:var(--cyan);letter-spacing:2px;font-family:'Syne',sans-serif;}
.nav-btn{
...
.hero{
  ...
  animation:fadeIn 1.5s ease;padding:40px;
}
.hero h1{
  font-size:clamp(30px,5.5vw,54px);margin-bottom:16px;font-weight:800;
  font-family:'Syne',sans-serif;
  background:linear-gradient(90deg,var(--cyan),var(--indigo));
  -webkit-background-clip:text;color:transparent;
}
.hero p{font-size:17px;opacity:.6;margin-bottom:32px;font-family:'JetBrains Mono',monospace;}
.hero-btn{
  padding:14px 36px;border:none;border-radius:30px;
  background:linear-gradient(90deg,var(--cyan),var(--indigo));
  font-weight:700;cursor:pointer;font-size:15px;font-family:'Poppins',sans-serif;
  transition:.2s;box-shadow:0 0 30px rgba(0,242,255,0.3);color:#0b1120;
}
...
.login-box{
  background:var(--card);padding:40px 36px;border-radius:20px;width:360px;
  box-shadow:0 0 40px rgba(0,242,255,0.15);border:1px solid rgba(0,242,255,0.1);
}
.login-title{
  text-align:center;margin-bottom:24px;font-weight:800;color:var(--cyan);
  font-family:'Syne',sans-serif;font-size:20px;letter-spacing:1px;
}
.tab-bar{display:flex;gap:8px;margin-bottom:24px;}
.tab-btn{
  flex:1;padding:10px;border:1px solid rgba(0,242,255,0.3);background:transparent;
  color:var(--cyan);border-radius:10px;cursor:pointer;font-family:'Poppins',sans-serif;
  font-weight:600;font-size:14px;transition:.2s;
}
.tab-btn.active{background:var(--cyan);color:#0b1120;border-color:var(--cyan);}
.field{
  width:100%;padding:12px 14px;margin:8px 0;border:1px solid var(--surface);
  border-radius:10px;background:var(--surface);color:var(--text);
  font-family:'Poppins',sans-serif;font-size:14px;outline:none;transition:.2s;
}
.field:focus{border-color:var(--cyan);}
.btn-login{
  width:100%;background:linear-gradient(90deg,var(--cyan),var(--indigo));
  border:none;padding:12px 20px;border-radius:12px;margin-top:10px;
  cursor:pointer;font-family:'Poppins',sans-serif;font-weight:700;font-size:15px;
  transition:.2s;color:#0b1120;
}
...
.btn-nfc{
  ...
  padding:10px 24px;border-radius:12px;cursor:pointer;font-family:'Poppins',sans-serif;
  font-weight:600;font-size:14px;transition:.2s;
}
.btn-nfc:hover{background:rgba(0,242,255,0.08);}
.btn-nfc:disabled{opacity:.4;cursor:not-allowed;}

/* DASHBOARD — WA GUARDIAN STYLE */
#dashboard{animation:fadeIn .5s ease;}
.dash-header{
  display:flex;justify-content:space-between;align-items:center;
  padding:20px 60px;background:#111827;
  border-bottom:1px solid rgba(0,242,255,0.1);
  position:sticky;top:0;z-index:100;
}
.dash-logo{font-weight:700;font-size:20px;color:var(--cyan);font-family:'Syne',sans-serif;letter-spacing:1px;}
.dash-logout{
  background:none;border:1px solid var(--cyan);color:var(--cyan);
  padding:8px 22px;border-radius:20px;cursor:pointer;
  font-family:'Poppins',sans-serif;font-weight:600;transition:.2s;
...
.stats-strip{display:flex;gap:16px;margin-bottom:30px;flex-wrap:wrap;}
.stat-pill{
  flex:1;min-width:120px;background:#141b2d;border-radius:14px;padding:20px;
  text-align:center;box-shadow:0 0 16px rgba(0,242,255,0.07);
}
.stat-pill .num{font-size:32px;font-weight:700;font-family:'Syne',sans-serif;}
.stat-pill .lbl{font-size:12px;opacity:.55;font-family:'Poppins',sans-serif;margin-top:4px;}
.stat-pill.high .num{color:var(--danger);}
.stat-pill.med  .num{color:var(--warn);}
.stat-pill.low  .num{color:var(--ok);}

.wa-card{
  background:#141b2d;padding:30px;border-radius:15px;margin-bottom:30px;
...
.wa-card textarea{
  ...
  border:1px solid transparent;background:#0f1626;color:white;
  font-family:'Poppins',sans-serif;font-size:14px;resize:vertical;outline:none;transition:.2s;
}
.wa-card textarea:focus{border-color:rgba(0,242,255,0.35);}
.btn-scan{
  margin-top:15px;padding:12px 28px;background:var(--cyan);
  border:none;border-radius:30px;font-weight:600;cursor:pointer;
  font-family:'Poppins',sans-serif;font-size:14px;color:#0b1120;transition:.2s;
}
.btn-scan:hover{box-shadow:0 0 18px var(--cyan);transform:scale(1.03);}
.result-box{margin-top:20px;font-weight:600;font-size:17px;font-family:'Poppins',sans-serif;}
.risk-high{color:var(--danger);}
.risk-medium{color:var(--warn);}
.risk-low{color:var(--ok);}
.scan-bar{height:4px;margin-top:12px;border-radius:4px;overflow:hidden;background:var(--surface);}
...
.wa-log{
  font-size:13px;line-height:1.8;max-height:160px;overflow:auto;
  background:#0f1626;padding:14px;border-radius:10px;
  font-family:'JetBrains Mono',monospace;
}
.wa-card canvas{background:#0f1626;border-radius:10px;padding:10px;}
...
<section id="homePage">
  <header class="site-header">
    ...
    <div><button class="nav-btn" onclick="showLogin()">Login</button></div>
  </header>
  <div class="hero">
...
<section id="loginPage" class="hidden">
  <div class="login-box">
    ...
    <!-- PASSWORD TAB -->
    <div id="tabPassword">
      <input class="field" type="text"     id="user" placeholder="Username" />
...
<section id="dashboard" class="hidden">
  ...
  <div class="container">
    ...
    <div class="stats-strip">
      ...
      <div class="stat-pill med"> <div class="num" id="statMed">0</div> <div class="lbl">⚡ Medium Risk</div></div>
...
async function startNFCScan(){
  ...
  if(!("NDEFReader" in window)){
    ...
    uid.textContent   = "Gunakan Chrome 89+ di Android.";
    return;
  }
  try{
...
function handleNFCRead(rawUID){
  ...
  if(match){
    ...
    hint.textContent="Kartu tidak terdaftar.";
    if(nfcAbortCtrl) nfcAbortCtrl.abort();
    resetNFCBtn(2800);
  }
}
...
function resetNFCBtn(d=0){
  setTimeout(()=>{ const b=document.getElementById("nfcScanBtn"); if(b){b.disabled=false;b.textContent="Mulai Scan NFC";} },d);
}
...
const attackChart = new Chart(
  ...
  options:{
    plugins:{ legend:{ labels:{ color:"white", font:{family:"Poppins"} } } },
    scales:{
      x:{ ticks:{color:"white"}, grid:{color:"rgba(255,255,255,0.04)"} },
      y:{ ticks:{color:"white"}, grid:{color:"rgba(255,255,255,0.04)"}, beginAtZero:true }
    }
  }
});
